...
In this chapter we will discuss different ways to implement a home automation system in \fodot. The advantages and disadvantages of every implementation and what we will use going forward. To make it more understandable, we will use an example scenario shown in Figure~\ref{fig:fodot-structure_example}.
\begin{figure}
    \centering
...
\section{Vocabulary}
The vocabulary is an important part of the \fodot description as it defines the possibilities and limits for the rest of the description. There are lots of different aproaches possible, which needs to be effective with the idea of ease of use and effectiveness in a real-world application.
The first vocabulary we thought of is one that is easy to create from blocks due to the way devices are represented. Each device has its own constant, which is linked to an area. The states of these devices are represented through a type that defines all possible states for that device.
...
An important note to make is the fact that these vocabularies limit the user in some way or another, this is by how the \fodot language is desinged. However, since this is not just the \fodot description, some functionality (that is not possible in \fodot) can be implemented in the blockly backend. This is further explored in ....
